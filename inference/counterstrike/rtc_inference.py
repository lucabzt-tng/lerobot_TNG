...
def main(server_host: str, server_port: int):
    ...
    # Fetch the first chunk before entering the loop
    action_chunk = rtc_client.get_action_chunk(observation_dict)
    chunk_axes = action_chunk["action.axes"]      # (chunk_size, 6)
    chunk_buttons = action_chunk["action.buttons"]  # (chunk_size, num_buttons)
    chunk_idx = 0

    print(f"Initial action chunk obtained: {chunk_axes.shape[0]} actions")

    # Track the last executed action so it can be fed back as state on re-query
    ...
    while running:
        ...
        if (elapsed_time >= FRAME_DURATION):
            ...
            # Re-query the server only when the current chunk is fully consumed
            if chunk_idx >= len(chunk_axes):
                observation_dict = {
                ...
                action_chunk = rtc_client.get_action_chunk(observation_dict)
                chunk_axes = action_chunk["action.axes"]
                chunk_buttons = action_chunk["action.buttons"]
                chunk_idx = 0
